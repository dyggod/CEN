# PM2 进程管理完整指南

## 🎯 什么是 PM2？

PM2 是一个专业的 Node.js 进程管理器，可以让你的服务：
- ✅ 后台运行（不需要终端窗口）
- ✅ 自动重启（崩溃后自动恢复）
- ✅ 开机自启（电脑重启后自动运行）
- ✅ 日志管理（自动记录所有输出）
- ✅ 状态监控（CPU、内存使用情况）

## 🚀 快速开始

### 1. 安装 PM2

**方法 A：自动安装（推荐）**

直接双击 `PM2启动.bat`，首次运行会自动安装 PM2。

**方法 B：手动安装**

```bash
npm install -g pm2
```

验证安装：

```bash
pm2 --version
# 应该显示：5.x.x
```

---

### 2. 启动服务

**双击运行：** `PM2启动.bat`

或命令行：

```bash
cd node_server
npm run pm2:start
```

你会看到：

```
┌─────┬──────────────────┬─────────┬─────────┬──────────┬────────┬──────────┐
│ id  │ name             │ mode    │ ↺      │ status   │ cpu    │ memory   │
├─────┼──────────────────┼─────────┼─────────┼──────────┼────────┼──────────┤
│ 0   │ keltner-webhook  │ fork    │ 0       │ online   │ 0%     │ 30.5mb   │
└─────┴──────────────────┴─────────┴─────────┴──────────┴────────┴──────────┘
```

✅ **状态为 `online` 表示服务运行正常！**

---

### 3. 常用操作

#### 查看状态

**双击运行：** `PM2查看状态.bat`

或命令行：

```bash
pm2 status
# 或
pm2 info keltner-webhook
```

---

#### 查看日志

**双击运行：** `PM2查看日志.bat`

或命令行：

```bash
pm2 logs keltner-webhook

# 仅看最新 50 行
pm2 logs keltner-webhook --lines 50

# 清空日志
pm2 flush
```

**日志文件位置：**
- 输出日志：`node_server/logs/output.log`
- 错误日志：`node_server/logs/error.log`

---

#### 重启服务

**双击运行：** `PM2重启.bat`

或命令行：

```bash
pm2 restart keltner-webhook
```

**何时需要重启？**
- 修改了 `server.js` 代码
- 修改了配置参数
- 服务出现异常

---

#### 停止服务

**双击运行：** `PM2停止.bat`

或命令行：

```bash
pm2 stop keltner-webhook
```

⚠️ 停止后服务不会运行，不会响应 Webhook 请求。

---

#### 完全删除服务

**双击运行：** `PM2完全删除.bat`

或命令行：

```bash
pm2 delete keltner-webhook
```

⚠️ 删除后需要重新运行 `PM2启动.bat` 才能使用。

---

### 4. 设置开机自动启动（推荐）

**双击运行：** `PM2设置开机启动.bat`（需要管理员权限）

或命令行：

```bash
# 配置开机启动
pm2 startup

# 保存当前进程列表
pm2 save
```

**验证：**
1. 重启电脑
2. 打开浏览器访问：http://localhost:5000/health
3. 如果能看到响应，说明开机启动成功！

**取消开机启动：**

```bash
pm2 unstartup
pm2 save
```

---

## 📊 高级功能

### 实时监控

查看 CPU、内存等实时数据：

```bash
pm2 monit
```

显示类似：

```
┌─ Process List ────────────────┐┌─ keltner-webhook Metrics ─────┐
│                                ││ CPU Usage:      0.5%           │
│ [0] keltner-webhook            ││ Memory Usage:   31.2 MB        │
│ status: online                 ││ Restart:        0              │
│ cpu: 0%                        ││ Uptime:         2h 15m         │
│ memory: 30MB                   ││                                │
└────────────────────────────────┘└────────────────────────────────┘
```

---

### 查看详细信息

```bash
pm2 describe keltner-webhook
```

显示：
- 进程 ID
- 运行时间
- 重启次数
- 内存使用
- 日志文件路径
- 配置信息

---

### 管理多个服务

```bash
# 查看所有服务
pm2 list

# 重启所有服务
pm2 restart all

# 停止所有服务
pm2 stop all

# 删除所有服务
pm2 delete all
```

---

### 自定义配置

编辑 `ecosystem.config.js`：

```javascript
module.exports = {
  apps: [{
    name: 'keltner-webhook',
    script: './server.js',
    
    // 修改实例数量（负载均衡）
    instances: 1,
    
    // 自动重启
    autorestart: true,
    
    // 最大内存限制
    max_memory_restart: '200M',  // 改为 '500M' 提高限制
    
    // 最大重启次数
    max_restarts: 10,
    
    // 最小运行时间
    min_uptime: '10s',
    
    // 环境变量
    env: {
      NODE_ENV: 'production',
      PORT: 5000  // 改为其他端口
    }
  }]
};
```

修改后重启生效：

```bash
pm2 restart keltner-webhook
```

---

## 🔧 故障排除

### 问题 1：服务启动失败

**症状：**
```
status: errored
```

**解决：**

```bash
# 查看错误日志
pm2 logs keltner-webhook --err

# 常见原因：
# 1. 端口被占用 → 修改 ecosystem.config.js 中的 PORT
# 2. 依赖未安装 → 运行 npm install
# 3. 配置错误 → 检查 server.js 中的配置
```

---

### 问题 2：服务频繁重启

**症状：**
```
↺ restart: 5
```

**解决：**

```bash
# 查看日志找出原因
pm2 logs keltner-webhook --lines 100

# 常见原因：
# 1. 内存不足 → 增加 max_memory_restart
# 2. 代码错误 → 检查 error.log
# 3. 依赖问题 → npm install
```

---

### 问题 3：日志文件太大

**解决：**

```bash
# 清空所有日志
pm2 flush

# 或者安装日志轮转模块
pm2 install pm2-logrotate

# 配置日志轮转
pm2 set pm2-logrotate:max_size 10M
pm2 set pm2-logrotate:retain 7
```

---

### 问题 4：PM2 命令找不到

**症状：**
```
'pm2' 不是内部或外部命令
```

**解决：**

```bash
# 重新安装 PM2
npm install -g pm2

# 检查 npm 全局路径
npm config get prefix

# Windows: 确保以下路径在环境变量 PATH 中
# C:\Users\你的用户名\AppData\Roaming\npm
```

---

## 📝 日常使用流程

### 开发阶段

1. **启动服务：** 双击 `PM2启动.bat`
2. **查看日志：** 双击 `PM2查看日志.bat`（调试用）
3. **修改代码后：** 双击 `PM2重启.bat`

### 生产环境

1. **首次部署：**
   ```bash
   npm install
   双击 PM2启动.bat
   双击 PM2设置开机启动.bat
   ```

2. **更新代码后：**
   ```bash
   双击 PM2重启.bat
   ```

3. **日常检查：**
   ```bash
   pm2 status  # 检查运行状态
   pm2 logs    # 查看日志
   ```

---

## 🎯 对比：普通启动 vs PM2

| 功能 | 普通启动 (`npm start`) | PM2 (`pm2 start`) |
|------|----------------------|-------------------|
| 需要终端窗口 | ✅ 是 | ❌ 否 |
| 后台运行 | ❌ 否 | ✅ 是 |
| 崩溃自动重启 | ❌ 否 | ✅ 是 |
| 开机自启 | ❌ 否 | ✅ 是 |
| 日志管理 | ❌ 否 | ✅ 是 |
| 状态监控 | ❌ 否 | ✅ 是 |
| 适用场景 | 开发测试 | 生产使用 |

---

## 🌟 最佳实践

### 1. 使用 PM2 启动（推荐）

✅ **优点：**
- 不占用终端窗口
- 自动重启，稳定可靠
- 便于监控和管理

❌ **缺点：**
- 首次需要安装 PM2

### 2. 设置开机自启

✅ **适用场景：**
- 长期使用
- 无人值守
- 生产环境

### 3. 定期查看日志

```bash
# 每周检查一次
pm2 logs keltner-webhook --lines 100

# 清理旧日志
pm2 flush
```

### 4. 监控资源使用

```bash
# 定期查看
pm2 monit

# 如果内存持续增长 → 可能存在内存泄漏
# 解决：重启服务或增加 max_memory_restart
```

---

## 📞 常用命令速查表

| 操作 | 命令 | 快捷方式 |
|------|------|---------|
| 启动 | `pm2 start ecosystem.config.js` | `PM2启动.bat` |
| 停止 | `pm2 stop keltner-webhook` | `PM2停止.bat` |
| 重启 | `pm2 restart keltner-webhook` | `PM2重启.bat` |
| 删除 | `pm2 delete keltner-webhook` | `PM2完全删除.bat` |
| 状态 | `pm2 status` | `PM2查看状态.bat` |
| 日志 | `pm2 logs keltner-webhook` | `PM2查看日志.bat` |
| 监控 | `pm2 monit` | - |
| 详情 | `pm2 describe keltner-webhook` | - |
| 开机启动 | `pm2 startup && pm2 save` | `PM2设置开机启动.bat` |

---

## 🎉 总结

使用 PM2 后，你可以：

1. ✅ **关闭终端窗口** - 服务继续运行
2. ✅ **后台运行** - 不影响其他操作
3. ✅ **自动重启** - 崩溃后自动恢复
4. ✅ **开机自启** - 电脑重启后自动运行
5. ✅ **便于管理** - 一键启停、查看日志

**推荐做法：**
1. 首次使用：双击 `PM2启动.bat`
2. 设置开机启动：双击 `PM2设置开机启动.bat`
3. 以后忘掉它，服务会自动运行！

---

**版本：** 1.0.0  
**更新日期：** 2025-11-24  
**作者：** Keltner Signal Team

